...
Error * QuantileLossComputer::quantile_tids_errors_fast(RCover* cover) {
    ...
    if (N != sub_idx)
        std::cout << "Error in coverage" << std::endl;
    // std::cout << N << " = " << sub_idx << std::endl;

    double * s_under = new double[n_quantiles];
    double * s_above = new double[n_quantiles];
    ...
    for (int i = n_quantiles - 1; i >= 0; i--) {
        ...
        above[i] = ((double) (N - (h_low[i] + 1))) * y_pred[i] - above[i];
...
Error * QuantileLossComputer::quantile_tids_errors_slow(RCover* cover) {
    ...
    for (int j = 0; j < n_q; j++) {
        for (int i = 0; i < N; i++) {
            ...
            errors[j] += fmax(q * (y_pred[j] - y[i]), (q - 1) * (y_pred[j] - y[i]));
    ...
    std::cout << "\n";
    for (int i = 0; i < n_q; i++) {
        float q = cover->dm->getQuantile(i);
        float h = (N-1)*q;
        std::cout << "(" << floor(h) << ", " << ceil(h) << ") : " << ", ";
    }
    std::cout << "\n";
    for (int i = 0; i < n_quantiles; i++) {
        float q = cover->dm->getQuantile(i);
        float h = (N-1)*q;
        std::cout << y[(int) floor(h)] << ", ";
    }
        std::cout << "\n";
    for (int i = 0; i < n_quantiles; i++) {
        std::cout << y_pred[i] << ", ";
