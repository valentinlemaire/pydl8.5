...
class DL85QuantilePredictor(DL85Predictor):
    def __init__(
        ...
        if self.stop_after_better is not None and len(self.stop_after_better) != len(
            self.stop_after_better
        ):
    ...
    def quantile_error(self, y, tids, q):
        ...
        delta = (y_p - y[list(tids)]).astype(float)
        delta[delta > 0] *= 1 - q
        delta[delta < 0] *= (-q)
    ...
    def fit(self, X, y, sample_weight=None):
        ...
        if self.leaf_value_function is None:
            self.leaf_value_function = lambda tids, q: self.quantile_leaf_value(
                y[list(tids)], q
            )
        ...
        if self.sol_size == 2 + n_trees * 9:  # solution found
            for i in range(n_trees):
                self.trees_[i]["tree"] = json.loads(
                    solution[2 + i * 9].lstrip("Tree: ")
                )
                ...
                self.trees_[i]["lattice_size"] = int(
                    solution[2 + i * 9 + 5].split(" ")[1]
                )
                self.trees_[i]["runtime"] = float(solution[2 + i * 9 + 6].split(" ")[1])
                self.trees_[i]["timeout"] = bool(
                    strtobool(solution[2 + i * 9 + 7].split(" ")[1])
                )

                if self.trees_[i]["size"] >= 3 or self.max_errors[i] <= 0:
                    self.trees_[i]["accuracy"] = float(
                        solution[2 + i * 9 + 4].split(" ")[1]
                    )

                if (
                    self.trees_[i]["size"] < 3 and self.max_errors[i] > 0
                ):  # return just a leaf as fake solution
                    if not self.timeout_:
                        print(
                            "DL8.5 fitting: Solution not found. However, a solution exists with error equal to the "
                            "max error you specify as unreachable. Please increase your bound if you want to reach it."
                        )
                    else:
                        print(
                            "DL8.5 fitting: Timeout reached without solution. However, a solution exists with "
        ...
        if hasattr(self, "trees_"):
            for i in range(n_trees):
                if self.trees_[i]["tree"] is not None:
                    ...
                    leaf_fun = lambda tids: self.leaf_value_function(
                        tids, self.quantiles[i]
                    )
                    error_fun = lambda tids: self.quantile_error(
                        y, tids, self.quantiles[i]
                    )
    ...
    def predict(self, X):
        ...
        for i in range(X.shape[0]):
            ...
            for t in range(len(self.quantiles)):
                pred_per_quantile.append(
                    self.pred_value_on_dict(X[i, :], tree=self.trees_[t]["tree"])
                )
    ...
    def predict_proba(self, X):
        ...
        for i in range(X.shape[0]):
            ...
            for t in range(len(self.quantiles)):
                pred_per_quantile.append(
                    self.pred_proba_on_dict(X[i, :], tree=self.trees_[t]["tree"])
                )
            pred.append(pred_per_quantile)
